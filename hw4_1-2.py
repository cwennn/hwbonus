...
def reveal(row, col): #打開格子的函數
    global location, bomb_location, again, mine, first, flag_location
    if location[row][col] == "F": #撤掉旗子
        ...
        if again == "y":
            location = [[" " for i in range(9)] for j in range(9)]
            bomb_location = []
            mine = 10
            map()
            ...
            for i in range(max(row-1, 0), min(row+2, 9)):
                for j in range(max(col-1, 0), min(col+2, 9)):
                    if i != row or j != col:
                        reveal(i, j)
        return
location = [[" " for i in range(9)] for j in range(9)] #初始化
bomb_location = []
flag_location = []
mine = 10
map()
            ...
            for i in range(9):
                for j in range(9):
                    if [i, j] in bomb_location:
                        ...
                        flag = False
                        reveal(i, j)
            end_time = time.time() #紀錄最終時間
            total_time = end_time - start_time #花費時間
            minute, sec = total_time // 60, total_time % 60 #換成分秒
            print("You Win. It took you %d minutes and %d seconds." % (minute, sec))
            map()
            again = input("Play again? (y/n):") #是否重玩
            if again == "y":
                location = [[" " for i in range(9)] for j in range(9)]
                bomb_location = []
                mine = 10
